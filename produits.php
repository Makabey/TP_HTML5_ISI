...
/*
	En passant par des variables variable, attribuer soit la valeur trouvée dans $_GET (et vérifier qu'elle ne dépasse pas le Max) soit -1 si non trouvé
 */
...
/*
	Si iProduit_ID n'est pas vide, trouver à quelle catégorie l'objet appartient et attribuer à iProduit_Cat l'index trouvé
*/
if($iProduit_ID != -1){
...
?>
				...
				<img id="produits_infos_img" src="images/produits/broken_toy.jpg" alt="Catégorie vide" title="Catégorie vide" />
				<dl>
					<dt>Nom : </dt>
				...
				<div>
					...
					<div id="produits_infos_ajouterpanier">
						<label>Quantitée : <input type="number" value="<?=$iProduit_QTE?>" min="1" max="99" required="required" /></label><img class="cursor_hand icons_basket" id="produits_infos_prodid" src="images/shopping-cart-add1.png" alt="Ajouter au panier" title="Ajouter au panier" />
					</div>
					<?php } ?>
					<div id="produits_infos_btns">
						<button type="button" id="prod_btnPrev" class="btn">&lt; Précédent</button>
						<span></span>
						<button type="button" id="prod_btnNext" class="btn">Suivant &gt;</button>
					</div>
				</div>
<?php
