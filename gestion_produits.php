...
if($_GET['product_ID'] != -1){
	$retour = ecrireCSV_VersTblIdx($_GET, 'res/table_produits.csv');
	if($retour === false){
...
?>
					<form id="formGestionProduits" method="get" action="<?=$_SERVER['SCRIPT_NAME']?>" autocomplete="off">
							...
							<ul>
								...
								<li>
									<ul>
										...
										<li><input type="text" id="nom" name="nom" title="Nom entre 3 et 25 caractères, commençant par une majuscule" placeholder="Nom de l'item" required="required" pattern="[A-Z].{2,25}" value="<?=$_GET['nom']?>" /></li>
									</ul>
								</li>
								<li>
									<ul>
										<li><label for="prix">Prix :</label></li>
										<li><input type="text" id="prix" name="prix" title="format préféré : 5.2 ex: 31275.98" placeholder="ex: 12.95" required="required" pattern="[0-9]{1,5}\.?[0-9]{0,2}\$?" value="<?=$_GET['prix']?>" /></li>
									</ul>
								</li>
								<li>
									<ul>
										<li><label for="nbrEnInventaire">Nombre en inventaire :</label></li>
										<li><input type="number" id="nbrEnInventaire" name="nbrEnInventaire" title="Entre 0 et 32767" min="0" max="32767" required="required" value="<?=$_GET['nbrEnInventaire']?>" /></li>
									</ul>
								</li>
								<li>
									<ul>
										<li><label for="gestion_fiche_description">Description :</label></li>
										<li><textarea id="gestion_fiche_description" name="description" title="(optionel) Entrez une description de 3 à 240 caractères, commençant par une majuscule" placeholder="(optionel) Entrez une description de 3 à 240 caractères, commençant par une majuscule" pattern="[A-Z].{2,240}" cols="94" rows="7" ><?=$_GET['description']?></textarea></li>
									</ul>
								</li>
								...
								<li>
									<ul>
										...
										<li><input type="file" id="fichierImage_browse" accept="image/*" />
											<input type="hidden" id="fichierImage" name="fichierImage" value="<?=$_GET['fichierImage']?>"/>
